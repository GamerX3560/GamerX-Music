...
internal class DataStoreManagerImpl(
    ...
    override val customBackgroundEnabled: Flow<String> = settingsDataStore.data.map { preferences ->
        preferences[CUSTOM_BACKGROUND_ENABLED] ?: FALSE
    }

    override suspend fun setCustomBackgroundEnabled(enabled: Boolean) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[CUSTOM_BACKGROUND_ENABLED] = if (enabled) TRUE else FALSE
            }
        }
    }

    override val customBackgroundPath: Flow<String> = settingsDataStore.data.map { preferences ->
        preferences[CUSTOM_BACKGROUND_PATH] ?: ""
    }

    override suspend fun setCustomBackgroundPath(path: String) {
        withContext(Dispatchers.IO) {
            settingsDataStore.edit { settings ->
                settings[CUSTOM_BACKGROUND_PATH] = path
            }
        }
    }
    ...
    companion object Settings {
        ...
        val CUSTOM_BACKGROUND_ENABLED = stringPreferencesKey("custom_background_enabled")
